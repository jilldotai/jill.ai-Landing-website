...
const LegalOverlay: React.FC<LegalOverlayProps> = ({ isOpen, onClose, title, content }) => {
  ...
  return (
    <div 
      ref={overlayRef}
      className="fixed inset-0 z-[200] bg-black/98 backdrop-blur-3xl flex flex-col opacity-0 invisible translate-y-5"
    >
      {/* Persistent Floating Close Button */}
      <button 
        onClick={onClose}
        className="fixed top-8 right-8 z-[210] w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:border-accent hover:bg-accent/10 transition-all duration-300 shadow-2xl backdrop-blur-md group"
